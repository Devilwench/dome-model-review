...
function p(text) {
  const runs = typeof text === 'string' ? [new TextRun(text)] : text.map(t => new TextRun(t));
...
function vRow(win, claim, verdict, flaw) {
  ...
  return new TableRow({ children: [
    new TableCell({ borders, width: { size: cw[0], type: WidthType.DXA }, margins: cellMargins,
      children: [new Paragraph({ children: [new TextRun({ text: win, bold: true, size: 18, font: "Arial" })] })] }),
...
C.push(new Paragraph({ spacing: { before: 600 }, alignment: AlignmentType.CENTER, children: [
  new TextRun({ text: "March 12, 2026  |  Version 2", size: 24, font: "Arial", color: "666666" }) ]}));
...
C.push(p("The table below uses five verdict categories. Detailed analysis of each claim follows, with sources and supporting data."));
...
// ══════════════════════════════════════════════════════════════════════
// DETAILED REVIEWS — selected key claims with strengthened evidence
// ══════════════════════════════════════════════════════════════════════
...
C.push(h3("WIN-001: Tesla 11.78 Hz Earth Resonance"));
C.push(p([b("Claim: "), { text: "US Patent 787412 contains formula f = c/(2*disc_thickness) giving 11.787 Hz." }]));
C.push(p([b("Evidence: "), { text: "US Patent 787412 (1905), publicly available at patents.google.com, describes electrical energy transmission through the Earth. The patent text mentions that the charge oscillates approximately twelve times per second but derives no disc-thickness frequency formula. The specific equation f = c/(2D) does not appear in the patent. Furthermore, 11.78 Hz is distinct from the Schumann resonance (7.83 Hz); conflating them misidentifies two separate electromagnetic phenomena." }]));
C.push(p([b("Verdict: REFUTED BY DATA. "), { text: "The patent citation is verifiably inaccurate." }]));

C.push(h3("WIN-008/009: Telluric 11.7-12 Hz Peak/Cutoff"));
...
C.push(h3("WIN-016: Annual Aberration via Dome Refraction"));
...
C.push(h3("WIN-017: Stellar Parallax as Firmament Wobble"));
...
C.push(h3("WIN-026: Crepuscular Ray Divergence"));
C.push(p([b("Claim: "), { text: "Rays visibly diverge from a local sun at approximately 5,733 km." }]));
C.push(p([b("Evidence: "), { text: "Crepuscular rays are parallel beams of sunlight made visible by atmospheric scattering. The apparent divergence is a perspective effect identical to railroad tracks appearing to converge. Crucially, anticrepuscular rays, visible in the direction opposite the Sun, appear to converge toward the anti-solar point. Both sets of rays (crepuscular and anticrepuscular) are visible simultaneously on rare occasions, forming parallel lines that appear to converge at both horizons. A truly local sun at 5,733 km could not produce rays that converge at both the solar and anti-solar points simultaneously." }]));
...
C.push(h3("WIN-002: Schumann 26% Aetheric Damping"));
...
C.push(h3("WIN-004/005: SAA Separation and Asymmetric Decay"));
C.push(p([b("Claim: "), { text: "Globe models have no mechanism for SAA splitting or asymmetric decay." }]));
C.push(p([b("Evidence: "), { text: "Terra-Nova et al. (2017, PNAS) demonstrate that reversed-flux patches at the core-mantle boundary, driven by differential rotation of the outer core, reproduce SAA splitting in magnetohydrodynamic simulations. The CHAOS-7 model (Finlay et al. 2020) documents the evolution quantitatively. ESA Swarm satellites continuously monitor the SAA from orbit at 460 km altitude, confirming the splitting pattern is consistent with outer-core convection models. The claim that globe models have no mechanism is factually incorrect." }]));
C.push(p([b("Verdict: STANDARD MODEL EXPLAINS. "), { text: "MHD simulations reproduce SAA splitting from outer-core dynamics." }]));

C.push(h3("WIN-010/025: Eclipse Magnetic Anomalies"));
C.push(p([b("Claim: "), { text: "-10.9 nT anomaly tracks eclipse geometry, not solar time, requiring aetheric explanation." }]));
C.push(p([b("Evidence: "), { text: "Eclipse-induced geomagnetic perturbations were predicted by Chapman (1933) via ionospheric conductivity drops. When the Moon blocks sunlight, photoionization ceases in the E-layer (90-150 km), reducing conductivity and disrupting the Sq current system. The disturbance naturally correlates with solar obscuration percentage (which follows eclipse geometry), not with fixed local time. This is exactly what Chapman's model predicts. The November 2024 multi-station paper confirms the pattern is consistent with ionospheric mechanisms across all 9 stations." }]));
C.push(p([b("Verdict: STANDARD MODEL EXPLAINS. "), { text: "Chapman ionospheric mechanism predicts geometry-tracking signal." }]));

C.push(h3("WIN-029: Schumann Requires Hard Conductive Ceiling"));
C.push(p([b("Claim: "), { text: "H = c/(4*7.83) = 9,572 km. Globe has no closed conductive cavity." }]));
C.push(p([b("Evidence: "), { text: "The formula used is for a quarter-wave linear waveguide, not a spherical cavity. The correct Schumann formula is f = c/(2*pi*R), giving R = c/(2*pi*7.83) = 6,098 km, within 4% of Earth's actual radius (6,371 km). The ionosphere IS a conductive boundary: it is a plasma with measurable conductivity mapped by satellites (CHAMP, Swarm, ISS ionospheric instruments). The globe model has exactly the closed conductive cavity the claim says it lacks. The correct formula actually confirms the globe's radius." }]));
...
C.push(h3("WIN-035-039: Weekly Confirmations"));
C.push(p([b("Claim: "), { text: "Five predictions registered March 6, confirmed March 12, 2026." }]));
C.push(p([b("Evidence: "), { text: "All five observations are phenomena that WMM2025, IGRF-13, and standard ionospheric models already predict. WMM2025 (released December 2024) publishes the NMP position at 139.298 deg E. SAA secular variation of ~25 nT/year is standard. Schumann resonance stability is expected (cavity dimensions unchanged). Lunar tidal magnetic signals are documented by Swarm/CHAMP satellite measurements as arising from ocean tidal dynamo effects. None of these outcomes would surprise a mainstream geophysicist. The predictions are analogous to predicting tomorrow's sunrise: timestamped in advance but trivially confirmable." }]));
...
C.push(h3("WIN-015: Meyl Scalar Wave Faraday Penetration"));
...
C.push(h3("WIN-011/012: Mohe Gravity Anomaly and Coupling Constant"));
...
C.push(p("WIN-003 (King's Chamber): Reid 1997 found 117 Hz acoustic resonance of the granite sarcophagus. This is an acoustic resonance of a stone chamber (dimensions 10.47m x 5.23m x 5.81m of Aswan granite), not an electromagnetic dome harmonic. WIN-007/022 (NP acceleration): counted as two separate wins using the same dataset. WIN-013/014 (SG nulls): claimed as wins alongside WIN-011, but if a gravity anomaly exists (WIN-011) then nulls (WIN-013/014) should falsify the model, not confirm it. WIN-018 (analemma 6.9 min): the equation of time ranges from +16.5 to -14.1 minutes per the U.S. Naval Observatory; 6.9 is the approximate RMS, a cherry-picked representation. WIN-023/031/032/034: theological assertions that cannot be tested by any physical measurement and should not count toward a scientific prediction tally."));
...
C.push(p([b("What the data shows: "), { text: "Eratosthenes (240 BC) measured this directly: at solar noon on the same day, the Sun was directly overhead at Syene (24.1 deg N) but 7.2 degrees from vertical at Alexandria (31.2 deg N), 800 km away. This difference in solar altitude corresponds precisely to the latitude difference. A local sun at 5,733 km would produce a much smaller angular difference (arctan(800/5733) = 7.95 degrees, close but only by coincidence for this particular pair). For more distant city pairs the discrepancy grows dramatically. Modern solar position calculators (NOAA SPA, SunCalc) predict sun altitude from any location using orbital mechanics, matching observations to arcsecond precision." }]));
C.push(p([b("Public data: "), { text: "NOAA Solar Position Algorithm; sunearthtools.com; U.S. Naval Observatory solar/lunar data." }]));
...
C.push(p([b("What the data shows: "), { text: "Radio telescopes routinely detect signals from pulsars (thousands of light-years), radio galaxies (billions of light-years), and fast radio bursts (FRBs) at distances exceeding 8 billion light-years. The CHIME telescope in Canada detects FRBs daily. These signals traverse the alleged dome location with no detectable attenuation signature, no scattering, no multipath interference, and no frequency-dependent absorption. A copper shell at any altitude would produce all of these effects. The Cosmic Microwave Background radiation is detected uniformly from all directions, inconsistent with a reflective dome that would create strong directional patterns." }]));
C.push(p([b("Public data: "), { text: "CHIME/FRB project (chime-frb.ca); ATNF Pulsar Catalogue; Planck CMB data (ESA)." }]));

C.push(h2("3.6 Horizon Distance and Curvature"));
C.push(p([b("The test: "), { text: "Measure horizon distance from known altitudes and observe hull-down effect for ships." }]));
C.push(p([b("What the data shows: "), { text: "The hull-down effect (ship hulls disappearing before masts as they recede) is observable with binoculars from any coastline. The distance to the horizon follows d = sqrt(2*R*h): from 10m elevation, horizon is 11.3 km; from 35,000 feet (aircraft cruise), horizon is 370-420 km. On a flat disc, the horizon would extend to the disc edge in every direction. Aircraft at cruise altitude would see the dome walls. Critically, curvature is symmetric in all directions from any observer, which is impossible on a flat disc where curvature would vary with azimuth from the pole." }]));
C.push(p([b("Public data: "), { text: "Any theodolite or transit measurement from coastal elevation; omnicalculator.com/physics/distance-to-horizon." }]));
...
C.push(p("NOAA World Magnetic Model 2025 (WMM2025): ncei.noaa.gov/products/world-magnetic-model"));
C.push(p("NOAA Wandering Geomagnetic Poles: ncei.noaa.gov/products/wandering-geomagnetic-poles"));
C.push(p("CHAOS-7 Geomagnetic Field Model: spacecenter.dk/files/magnetic-models/CHAOS-7"));
C.push(p("ESA Swarm Satellite Mission: earth.esa.int/eogateway/missions/swarm"));
C.push(p("INTERMAGNET Global Observatory Network: intermagnet.org"));
C.push(p("IGRF-13: ncei.noaa.gov/products/international-geomagnetic-reference-field"));
C.push(p("ESA Gaia Data Release 3 (1.8 billion stars): cosmos.esa.int/web/gaia/data-release-3"));
C.push(p("Hipparcos Catalogue: cosmos.esa.int/web/hipparcos"));
C.push(p("US Patent 787412 (Tesla): patents.google.com/patent/US787412A"));
C.push(p("NOAA Solar Position Algorithm: gml.noaa.gov/grad/solcalc"));
C.push(p("National Geodetic Survey Gravity Database: geodesy.noaa.gov"));
C.push(p("CHIME/FRB Project: chime-frb.ca"));
C.push(p("ATNF Pulsar Catalogue: atnf.csiro.au/research/pulsar/psrcat"));
C.push(p("Planck CMB Data (ESA): cosmos.esa.int/web/planck"));
C.push(p("GPS.gov: gps.gov"));
C.push(p("CelesTrak TLE Data: celestrak.org"));
...
const out = "/sessions/intelligent-tender-mccarthy/mnt/outputs/dome-cosmologic-model/critical-review-dome-model-v2.docx";
Packer.toBuffer(doc).then(buf => { fs.writeFileSync(out, buf); console.log("Written: " + out); }).catch(e => console.error(e));
