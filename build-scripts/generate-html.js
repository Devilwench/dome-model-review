...
<details id="eg-verdicts"><summary class="ps-summary"><h2 style="display:inline;margin:0">Verdict Assignment Criteria</h2><p class="ps-tldr">Each WIN receives the verdict matching its primary failure mode. When a WIN fails in multiple ways, the failure that would persist even if others were corrected takes priority. The six categories are not ranked by severity — they describe different kinds of failure, not degrees of it.</p></summary><div class="ps-detail">
...
<p><strong>Condition 1: Derive a discriminating prediction from dome geometry.</strong> Use the model's own stated parameters — H(r) = 8,537 × exp(−r/8,619), disc radius 20,015 km, κ = 1.67 nT/μGal — to derive a quantitative prediction that <em>differs</em> from what standard physics predicts for the same observable. Then confirm it with independent measurement. Currently only 2 of ${counts.total} claims even attempt a dome-geometry derivation (WIN-044, WIN-057), and both produce values that either match standard physics or contradict measurement.</p>
