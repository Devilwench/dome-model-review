...
<a href="assets/tesla-patent-787412-full.png" target="_blank"><img src="assets/tesla-patent-787412-fig1.png" alt="Tesla Patent 787412, Figure 1 — Earth depicted as a sphere with transmitter and receiver at diametrically opposite points" style="width:100%;display:block"></a>
<div style="padding:.5rem .7rem;font-size:.8rem;color:#555;line-height:1.3"><strong>US Patent 787,412, Fig. 1</strong> (N. Tesla, 1905). The Earth (labeled E) is depicted as a sphere. The transmitter (coil C, with terminals A and B) sends waves through and around the spherical Earth to receiver E' at the diametrically opposite point. <a href="https://patents.google.com/patent/US787412A" target="_blank" style="color:var(--accent)">View full patent →</a></div>
</div>
...
<p><strong>4. The blockchain timestamping doesn't prove what it claims.</strong> OpenTimestamps anchors <code>status_history.json</code> — the file containing <em>audit results</em> (observed values compared to predictions). But this bundles predictions and observations into the same timestamped document. To prove a prediction existed before an outcome, you need to timestamp the prediction <em>separately</em> from the observation. The prediction parameters live in <code>monitor.py</code> source code and <code>docs/model.html</code>, which are git-versioned but not blockchain-timestamped. Meanwhile, the source data (NOAA Kp, USGS earthquakes, NMP drift) is already publicly archived by those agencies with their own timestamps — blockchain-stamping a derivative record adds nothing for provenance. The current setup timestamps a combined blob that could theoretically be regenerated after the fact, defeating the purpose. To be scientifically meaningful, the system would need to timestamp a predictions-only document (parameters, formulas, expected values) separately from the audit results, and link to the provider's own data archives for independent verification of the observations.</p>
