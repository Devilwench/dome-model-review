...
<h2>3.5.2 The Disc Diameter Problem: 40,030 km vs. 28,210 km</h2>

<p><strong>WIN-062 (Tesla longitudinal wave speed):</strong> Calculates v_a = 40,030 km / 0.08484 s = 471,829 km/s = 1.574c. The disc diameter used is 40,030 km. But the dome's stated equatorial radius is r_eq = 14,105 km, giving a diameter of ~28,210 km. Where does 40,030 come from?</p>

<p><strong>40,030 km is suspiciously close to 40,075 km — Earth's equatorial circumference on the globe model.</strong> If the dome's disc diameter is derived from the globe's circumference, this is another globe value being relabeled as a dome parameter. The dome's own geometry gives a different number (28,210 km from the equatorial ring, or ~40,030 km from 2 × 20,015 km ice wall radius). The page does not explain the discrepancy or specify which dome measurement yields 40,030 km. A model that cannot state its own disc diameter without ambiguity has a foundational problem.</p>

<p><strong>The superluminal claim:</strong> WIN-062 asserts a wave speed of 1.574c (57.4% faster than light). The justification is that Tesla's signals were "longitudinal, not transverse EM" and therefore not subject to the speed-of-light limit. No physics is cited for why longitudinal waves in an aetheric medium would exceed c. This is an assertion, not a derivation. Special relativity's speed limit applies to all information-carrying signals regardless of wave type — this is experimentally confirmed to extraordinary precision by particle accelerators, GPS, and the Michelson-Morley experiment and its modern replications.</p>
...
<p>The homepage headlines "95.2% Accuracy." <strong>The repository source code reveals this figure is hardcoded as static HTML</strong> — it appears as <code>&lt;div class="score-number score-green"&gt;95.2%&lt;/div&gt;</code> in the homepage markup. No script anywhere in the repository computes it.</p>

<p><strong>We checked every plausible source file:</strong> <code>scoring.js</code> (per-prediction point-based scoring, no aggregate), <code>predictions.js</code> (prediction data, no percentage), <code>build.js</code> (hash generation only), <code>analytics.js</code> (browser telemetry only), <code>apply_scoring_schema.py</code>, <code>recalc_v51.py</code>, <code>compile_exhaustive_api.py</code>, <code>verify_predictions.py</code>, <code>build_tracking.py</code> — none contain a function that calculates an overall accuracy percentage. The figure has no derivation, no formula, and no audit trail in the codebase.</p>

<p><strong>It also cannot be reproduced from the repo's own data.</strong> The <code>api/scorecard.json</code> shows 26 confirmed, 1 falsified, 20 pending — giving 26/27 = 96.3%. The <code>api/current/results.json</code> shows 32 confirmed + 1 strong out of 33 resolved = 97.0%. The homepage claims 67 WINs with 4 refined and at least 4 acknowledged falsifications on the tracking page: 67/75 = 89.3%, or 71/75 = 94.7%. None of these produce 95.2%. The number does not follow from any combination of counts in the repository.</p>

<p>More fundamentally, accuracy metrics are meaningful only when the predictions are independent and discriminating. As our <a href="#part2" onclick="showTab('wins');return false">point-by-point review</a> shows, many WINs are re-sliced versions of the same data (WIN-040 through 043 are all SAA positioning from the same CHAOS-7 dataset), known constants (tidal periods), or globe values relabeled (Hubble Law, WGS84 gravity). Counting each as an independent "confirmed prediction" inflates both the numerator and the headline. But the most basic problem is simpler: <strong>the headline number is not computed from anything — it is typed into the HTML.</strong></p>
