...
build-scripts/apply-patches.js    # Applies decider patches against parsed JSON field values
...
monitor/decisions/open-issues.json # Persistent issue tracker (open only, ~178 issues)
monitor/decisions/closed-issues.json # Archive of fixed/wontfix issues
...
digest-reviews.js → pending-digest.json (compact summary of all unprocessed reviews)
         ↓
Decider reads digest → creates issues (or runs backfill-issues.js) → writes suggested-patches-*.json
         ↓
