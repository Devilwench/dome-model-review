...
- `monitor/decisions/prop-009-enforce.flag` — enforcement toggle for PROP-009r2 queue_id strict matching. Presence = enforce mode, absence = shadow mode. Classified `git` in both `build.js` OWNERSHIP and `workspace-sync.md` OWNED_BY_GIT.
